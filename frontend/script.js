...
function initializeApp() {
  ...
  document
    .querySelectorAll(".hero-card, .form-group, .nutrition-card, .meal-card")
    .forEach((el) => {
      observer.observe(el);
    });
  ...
  try {
    ...
    if (nutritionResponse.success) {
      ...
      if (mealsResponse.success) {
        ...
        showResults();
        showNotification(
          "Nutrition targets calculated successfully, but meal recommendations are currently unavailable.",
          "warning"
        );
      }
    }
...
async function getMealRecommendations(userData) {
  ...
  if (!response.ok) {
    if (response.status === 503) {
      ...
      return {
        success: false,
        message: "Meal recommendation service is currently unavailable",
      };
    }
    throw new Error(`HTTP error! status: ${response.status}`);
  }
...
<div class="nutrition-label">Protein (${
    macronutrients.protein.percentage
  }%)</div>
</div>
<div class="nutrition-card carbs">
<i class="fas fa-bread-slice"></i>
<div class="nutrition-value">${Math.round(
    macronutrients.carbohydrates.grams
  )}g</div>
<div class="nutrition-label">Carbs (${
    macronutrients.carbohydrates.percentage
  }%)</div>
</div>
...
function displayMealRecommendations(response) {
  if (
    !response.success ||
    !response.recommended_meals ||
    response.recommended_meals.length === 0
  ) {
...
`;
  ...
  const mealsHTML = response.recommended_meals
    .map((meal, idx) => {
      ...
      return `
<div class="meal-card slide-up" data-mealid="${mealId}" data-meal='${JSON.stringify(
        safeMealForPlan(meal)
      ).replace(/'/g, "&apos;")}'>
...
${
  hasMore
    ? `<button class="expand-ingredients" onclick="event.stopPropagation(); toggleIngredients('${mealId}')">More</button>`
    : ""
}
${
  hasMore
    ? `<div class="ingredients-hidden" id="ingredients-hidden-${mealId}" style="display:none;">${hidden
...
<button class="add-plan-button" onclick='event.stopPropagation(); addMealToPlan(${JSON.stringify(
        safeMealForPlan(meal)
      )})'>
...
`;
  const mainContainer =
    document.getElementById("recommendationMain") || mealRecommendations;
...
function addMealToPlan(meal) {
  ...
  // Update grocery map with measurements
...
function removeMealFromPlan(index) {
  ...
  if (removed) {
    ...
    normalizeIngredients(removed.ingredients || []).forEach((name) => {
      ...
      if (cur <= 1) groceryMap.delete(key);
      else groceryMap.set(key, cur - 1);
    });
    renderPlanSidebar();
...
function renderPlanSidebar() {
  ...
  const dailyTargets = currentNutritionTargets
    ? {
        calories: Math.round(
          currentNutritionTargets.calculations?.target_calories || 0
        ),
        protein: Math.round(
          currentNutritionTargets.macronutrients?.protein?.grams || 0
        ),
        carbohydrates: Math.round(
          currentNutritionTargets.macronutrients?.carbohydrates?.grams || 0
        ),
        fat: Math.round(
          currentNutritionTargets.macronutrients?.fat?.grams || 0
        ),
      }
    : null;
  ...
  const groceryItems = Array.from(groceryMap.entries())
    ...
    .map(
      ([name, count]) =>
        `<li>
<span>${name}</span>
${count > 1 ? ` <small>(${count} recipes)</small>` : ""}
</li>`
    )
    .join("");
...
<div class="macro-row"><span>Calories</span><strong>${
    totals.calories
  }</strong>${
    dailyTargets ? `<small>/ ${dailyTargets.calories}</small>` : ""
  }</div>
<div class="macro-row"><span>Protein</span><strong>${
    totals.protein
  } g</strong>${
    dailyTargets ? `<small>/ ${dailyTargets.protein} g</small>` : ""
  }</div>
<div class="macro-row"><span>Carbs</span><strong>${
    totals.carbohydrates
  } g</strong>${
    dailyTargets ? `<small>/ ${dailyTargets.carbohydrates} g</small>` : ""
  }</div>
<div class="macro-row"><span>Fat</span><strong>${totals.fat} g</strong>${
    dailyTargets ? `<small>/ ${dailyTargets.fat} g</small>` : ""
  }</div>
</div>
...
${
  groceryItems
    ? `<ul>${groceryItems}</ul>`
    : "<p><em>No items yet. Add meals to build your list.</em></p>"
}
</div>
</div>
`;
...
function normalizeIngredients(rawList) {
  return (rawList || [])
    .map((x) => {
      ...
      // Combine ingredient name with measure
      return x.measure && x.ingredient
        ? `${x.measure} ${x.ingredient}`.trim()
        : (x.name || "").trim();
    })
    .filter(Boolean);
}
...
function attachMealCardClickHandlers() {
  ...
  grid.addEventListener("click", (e) => {
    ...
    if (
      target.closest(".expand-ingredients") ||
      target.closest(".expand-button") ||
      target.closest(".add-plan-button") ||
    ...
    try {
      const mealData = JSON.parse(
        card.getAttribute("data-meal").replace(/&apos;/g, "'")
      );
...
function setLoadingState(isLoading) {
  ...
  if (isLoading) {
    ...
    showLoadingOverlay(
      "Calculating your personalized nutrition targets and meal recommendations..."
    );
  } else {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const cards = document.querySelectorAll(
    ".hero-card, .nutrition-card, .meal-card"
  );
  cards.forEach((card) => {
